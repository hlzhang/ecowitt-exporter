...
pub fn update_sensor_gauges(report: &Report<'_>, prom: &State<PromObjects>) {
    ...
    match report.eventrainin {
        Some(depth) => {
            prom.rainfall_in.with_label_values(&["event"]).set(depth);
            prom.rainfall_mm
    ...
    match report.hourlyrainin {
        Some(depth) => {
            prom.rainfall_in.with_label_values(&["hourly"]).set(depth);
            prom.rainfall_mm
    ...
    match report.dailyrainin {
        Some(depth) => {
            prom.rainfall_in.with_label_values(&["daily"]).set(depth);
            prom.rainfall_mm
    ...
    match report.weeklyrainin {
        Some(depth) => {
            prom.rainfall_in.with_label_values(&["weekly"]).set(depth);
            prom.rainfall_mm
    ...
    match report.monthlyrainin {
        Some(depth) => {
            prom.rainfall_in.with_label_values(&["monthly"]).set(depth);
            prom.rainfall_mm
    ...
    match report.yearlyrainin {
        Some(depth) => {
            prom.rainfall_in.with_label_values(&["yearly"]).set(depth);
            prom.rainfall_mm
    ...
    match report.wh40batt {
        Some(batt) => {
            prom.battery.with_label_values(&["wh40"]).set(batt);
        }
        _ => {
            prom.battery.remove_label_values(&["wh40"]).unwrap_or(());
        }
    };
}
